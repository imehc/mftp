...
export async function checkForUpdateManually() {
  ...
  try {
    ...
    if (!update) {
      resetUpdaterState();
      toast.success(translate(msg`当前已是最新版本`), {
        id: UPDATE_TOAST_ID,
      });
      return;
    }
...
function showAvailableUpdateToast() {
  ...
  toast.info(translate(msg`发现新版本 ${stateVersion}`), {
    id: UPDATE_TOAST_ID,
...
function showDownloadProgressToast() {
  ...
  const stateVersion2 = state.version;
  const stateVersion3 = state.version;
  toast.loading(
    state.phase === "installing"
      ? translate(msg`正在准备版本 ${stateVersion2}`)
      : translate(msg`正在下载版本 ${stateVersion3}`),
    {
      id: UPDATE_TOAST_ID,
...
function showReadyToRestartToast() {
  ...
  const stateVersion4 = state.version;
  toast.success(translate(msg`版本 ${stateVersion4} 已准备好`), {
    id: UPDATE_TOAST_ID,
...
function downloadProgressContent(
  ...
  if (state.phase === "installing")
    return translate(msg`下载完成，正在安装更新…`);
  if (!state.total) {
    const formatBytesValue = formatBytes(state.downloaded);
    return translate(msg`已下载 ${formatBytesValue}`);
  }
  const percent = Math.min(
    100,
    Math.round((state.downloaded / state.total) * 100),
  );
  const formatBytesValue2 = formatBytes(state.downloaded);
  const formatBytesValue3 = formatBytes(state.total);
  return translate(
