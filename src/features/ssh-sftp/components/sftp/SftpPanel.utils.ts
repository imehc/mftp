...
export function computeInitialSftpColumnSizing(
  ...
  // ResizeObserver 可能报告带小数的 CSS 像素。保持分配运算为整数，
  // 使余数循环不会在零附近无限振荡（例如 0.33 → -0.67 → 0.33）。
  ...
  for (const key of Object.keys(scaled)) {
    scaled[key] = Math.min(
      maxSizes[key] ?? scaled[key],
      Math.max(minSizes[key] ?? 0, scaled[key]),
    );
  }
  let diff = Math.round(
  ...
  while (diff !== 0) {
    ...
    for (const key of order) {
      ...
      if (next < (minSizes[key] ?? 0) || next > (maxSizes[key] ?? Infinity))
        continue;
      scaled[key] = next;
      diff -= Math.sign(diff);
      adjusted = true;
