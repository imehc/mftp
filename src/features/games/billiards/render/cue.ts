...
export function createCueView() {
  ...
  const strip = (top: number, bottom: number, color: number, alpha: number) => {
    stick
      .poly([
        0, halfWidthAt(0) * top,
        stickLength, halfWidthAt(stickLength) * top,
        stickLength, halfWidthAt(stickLength) * bottom,
        0, halfWidthAt(0) * bottom,
      ])
      .fill({ color, alpha });
  };
  ...
  band(stick, px(0.038), px(0.60), 0xd7b183);
  band(stick, px(0.60), px(0.638), 0x2c2c2c);
  band(stick, px(0.638), px(0.80), 0x6d3a23);
  band(stick, px(0.80), px(0.95), 0x3b2b21);
  band(stick, px(0.95), stickLength - px(0.022), 0x5b2f1e);
  band(stick, stickLength - px(0.022), stickLength, 0x1b1b1b);
  band(stick, px(0.792), px(0.80), 0xcfc9ba);
