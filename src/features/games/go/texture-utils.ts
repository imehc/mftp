/**
 * 围棋棋盘的程序化 Pixi 纹理：带种子的榧木盘面（每次启动一致）与
...
// 纹理在应用的整个生命周期内缓存：生成是开销最大的部分，而 v8 的
// TextureSource 会自行重新上传到需要的渲染器，因此能存活于 Application
// 卸载。因此销毁时务必不要向 app.destroy 传入 `texture: true`。
