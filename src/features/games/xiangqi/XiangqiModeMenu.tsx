...
export function XiangqiModeMenu({
  ...
  const records = useGameHistory<XiangqiHistoryPayload>(XIANGQI_GAME_ID);
  const clearGame = useGamesHistoryStore((state) => state.clearGame);
      ...
      <div className="flex w-full max-w-sm flex-col gap-2 self-center">
        <div className="flex flex-col gap-2 rounded-md border border-border p-2.5">
          ...
          <ToggleGroup
            ...
            className="w-full"
          >
            <ToggleGroupItem value="easy" className="flex-1"><Trans>简单</Trans></ToggleGroupItem>
            <ToggleGroupItem value="medium" className="flex-1"><Trans>中等</Trans></ToggleGroupItem>
            <ToggleGroupItem value="hard" className="flex-1"><Trans>困难</Trans></ToggleGroupItem>
          </ToggleGroup>
          <ToggleGroup
            type="single"
            variant="outline"
            ...
            onValueChange={(value) => {
              if (value) setLocalSeat(Number(value) as SeatIndex);
            }}
            className="w-full"
          >
            <ToggleGroupItem value="0" className="flex-1"><Trans>先手（执红）</Trans></ToggleGroupItem>
            <ToggleGroupItem value="1" className="flex-1"><Trans>后手（执黑）</Trans></ToggleGroupItem>
          </ToggleGroup>
        </div>
        <Button
          variant="outline"
        ...
        >
          ...
          <span className="text-xs font-normal text-muted-foreground"><Trans>同屏轮流</Trans></span>
        </Button>
        <Button
          variant="outline"
          size="sm"
        ...
        >
          ...
          <span className="text-xs font-normal text-muted-foreground"><Trans>局域网</Trans></span>
        </Button>
        {records.length > 0 ? (
          <div className="flex flex-col gap-1 rounded-md border border-border p-2.5">
            <div className="flex items-center justify-between">
              <span className="text-xs font-medium text-muted-foreground"><Trans>历史记录</Trans></span>
              <Button variant="ghost" size="xs" onClick={() => clearGame(XIANGQI_GAME_ID)}>
                <Trans>清空</Trans>
              </Button>
            </div>
