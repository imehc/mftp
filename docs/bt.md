...
文件目录树（名称 / 大小 / 类型 / 可播标记）
   │
   ├─ 勾选 + 「下载」 ──> output = 用户目录下的 .mftp-part/{infohash}/，完成后改名迁出
   └─ 单文件「预览」 ──> output = cache 目录，流式播放
```

下载不直接写用户目录：`FilesystemStorage::init` 会把勾选文件按完整长度 `set_len`，所以下载中的文件是个全尺寸稀疏占位（空洞读作 0），大小看着对但没有可用的容器头，任何播放器都打不开。因此普通下载先落到目标目录内的隐藏暂存目录 `.mftp-part/{infohash}/`，`finalize_direct` 在下完后 `session.delete(hash, false)` 再改名迁出——同盘改名是瞬时的，不产生第二份数据，用户目录里出现文件的那一刻它就是完整的。多选打包沿用应用数据区的 staging 目录。

代价：迁出即停止做种；单文件按原名平铺到目标目录（不保留种子的外层文件夹），同名追加 ` (n)`；旧版本遗留的 `work_dir == dest_dir` 任务仍在原地收尾。
