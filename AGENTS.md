...
本项目重度依赖 Tauri IPC。类型由 `specta + tauri-specta` 从 Rust 自动生成，**不要手写或手改前端 IPC 类型**。

- 绑定产物：`src/bindings.ts`，由 `src-tauri/src/lib.rs` 的 `specta_builder()` 导出，文件头写明「Do not edit this file manually」。
- `src/types.ts` 只是从 `bindings.ts` 再导出的门面，方便 `~/types` 短路径引用；新增类型改 Rust，不改这两个文件。
- 新增命令要做两件事：加 `#[tauri::command] #[specta::specta]`，并加进 `lib.rs` 的 `collect_commands![]`。漏第二步命令不会出现在前端。
- 事件 payload 类型不经过命令签名，必须在 `specta_builder()` 里显式 `.typ::<T>()` 注册，否则 `bindings.ts` 里没有该类型。
- 绑定在 `cargo test` 时会重新导出（`tests::export_typescript_bindings`），后端改完跑测试即可发现不同步。
- `collect_commands![]` 的条目上不能写 `#[cfg]`。平台专属命令需用局部 macro 包一层再按 `cfg` 选列表，见 `lib.rs` 的 `all_commands!`。
- IPC 调用集中在 `src/lib/ipc.ts`，不要在组件里直接 invoke。
- 事件名集中在 `src/lib/events.ts`，禁止内联字符串字面量。动态事件（如按 session 分段的 `ssh://data/{id}`）导出为函数，不要在调用处拼字符串。
- 命令命名保持 `动词_名词` 风格，例如 `list_hosts`、`start_transfer`。

## 前端分层

- `src/routes/**` 只做薄壳：路由定义、平台守卫、把参数交给 feature 组件，不写业务逻辑。
- `src/features/<name>/` 承载该功能的页面骨架、hooks、store、子组件。
- `src/lib/**` 放跨 feature 的纯工具（`format.ts`、`events.ts`、`ipc.ts`、`platform.ts`）。同一实现出现第二处时就该抽到这里——`formatBytes` 曾散落 5 份且行为不一致（有的 GB 封顶、有的无 NaN 守卫）。
- 平台守卫用 `src/lib/platform.ts` 的现成函数，不要在组件里自行判断。
