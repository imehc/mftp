    ...
    pub fn export_sections(&self, sections: &[ExportSection]) -> AppResult<Value> {
        ...
        for section in sections {
            let value = match section {
                ...
                ExportSection::AiTranslations => json!(self.all_poetry_translations()?),
            };
...
mod tests {
    ...
    fn ai_translation_export_round_trips_without_connection_config() {
        let source_root =
            std::env::temp_dir().join(format!("mftp-ai-export-{}", uuid::Uuid::new_v4()));
        let target_root =
            std::env::temp_dir().join(format!("mftp-ai-import-{}", uuid::Uuid::new_v4()));
